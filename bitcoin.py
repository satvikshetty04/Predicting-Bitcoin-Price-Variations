...
def computeDelta(wt, X, Xi):
    ...
    c = wt
    M = len(X)-1
    prediction, numerator, denominator = 0.0, 0.0, 0.0
    mean_x = np.mean(X[:-1])
    std_x = np.std(X[:-1])

    for row_no, x_i in Xi.iterrows() :
        mean_xi = np.mean(x_i[:-1])
        std_xi = np.std(x_i[:-1])
        yi = x_i[-1]
        temp = 0.0
        for k in range(0,M):
            temp += (X[k] - mean_x) * (x_i[k] - mean_xi)
        similarity = temp/(M * std_x * std_xi)
        factor = math.exp(c * similarity)
        numerator += yi*factor
        denominator += factor
